...
class UserService:
    ...
    def perform_second_request_with_errors_identification(self, errors_identification: list[str],
        ...
        try:
            if not self.data_validator.verify_valid_raw_itinerary(json_itinerary, self.required_request_keys.get('country-code')):
                logger.error(f"UsersService: second raw itinerary is not valid: {raw_itinerary},"
    ...
    @staticmethod
    def get_published_business_ids_from_itinerary(trip_itinerary: GeneratedTrip, recommendations) -> list[str]:
    ...
    def update_db_regarding_itinerary(self, trip_itinerary: GeneratedTrip, recommendations) -> Any:
    ...
    def build_trip_itinerary_response(self, response_json) -> 'GeneratedTrip':
        trip_days = []
    ...
    def get_a_valid_itinerary(self, recommendations):
        """
            ...
            try:
                ...
                try:
                    json_itinerary = self.perform_second_request_with_errors_identification(
                        [INVALID_JSON_STR], ErrorType.INVALID_JSON, c_name)
                except CouldNotGetValidResponseFromThirdParty as e:
                    raise e
    ...
    def build_trip(self) -> 'ResponseBuilder':
        ...
        # validate the response from the generative AI
        # todo: try switch to while not valid loop
        while not self.data_validator.verify_valid_raw_itinerary(json_itinerary, c_name) or \
                not self.data_validator.verify_all_fields_contain_data(json_itinerary, c_name):
            logger.info(f"UsersService: first raw itinerary is not valid: {raw_itinerary},"
                        f" found errors:\n{self.data_validator.errors}\n, trying to perform another request\n")
            try:
                json_itinerary = self.convert_ai_response_to_json(raw_itinerary)
        if not self.data_validator.verify_valid_raw_itinerary(json_itinerary, c_name):
            ...
            try:
                json_itinerary = self.perform_second_request_with_errors_identification(
                    self.data_validator.errors, ErrorType.INVALID_RESPONSE, c_name)
            except CouldNotGetValidResponseFromThirdParty as e:
                raise e
        ...
        self.update_db_regarding_itinerary(trip_itinerary, recommendations)
        # and secondly - build the response and return it
        return self.response_builder.build_business_response(trip_itinerary, 200, "trip_itinerary")
